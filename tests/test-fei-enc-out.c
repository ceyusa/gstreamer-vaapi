...
{
  GstElement *pipeline, *filesrc, *videoparse, *enc, *appsink;
  GError *err = NULL;
  ...
  gst_bin_add_many (GST_BIN (pipeline), filesrc, videoparse, enc, appsink,
      NULL);

  link_ok = gst_element_link_many (filesrc, videoparse, enc, appsink, NULL);
  if (!link_ok) {
